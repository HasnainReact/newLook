...
const App = ({ navigation }) => {
    ...
    const handleValidUserId = (text, key)=>{
        ...
        if((key==0) && (text==null || text=="")){ //Make first row field is required
            _userIdValid[key].userIdError = 'Required!';
        } else {
            _userIdValid[key].userId = (text);
    ...
    const inputHandleStartAmount = (text, key)=>{
        ...
        _startAmountInputs[key].startAmount = (text);
    ...
    const handleValidStartAmount = (text, key)=>{
        ...
        if((key==0) && (text==null || text=="")){ //Make first row field is required
            ...
            _startAmountValid[key].startAmount = (text);
    ...
    const inputHandleEndAmount = (text, key)=>{
        ...
        _endAmountInputs[key].endAmount = (text);
    ...
    const handleValidEndAmount = (text, key)=>{
        ...
        if((key==0) && (text==null || text=="")){ //Make first row field is required
            ...
            _endAmountValid[key].endAmount = (text);
    ...
    function validateFormMultipleFields(){
        ...
        formInputs.map((input, key)=>{
            ...
            if(input.userIdError!="" || input.startAmountError!="" || input.endAmountError!=""){
                isValid = false;
                return;
            }
        });
        ...
        <SafeAreaView style={styles.safeAreaContainer}>
            <ScrollView contentContainerStyle={styles.scrollView}>
                <View style={styles.mainContainer}>
                    <Text style={{fontSize: 16, fontWeight: 'bold', marginTop: 30, marginBottom: 30}}>Multiple Input Fields with Add more</Text>
                    <View style={styles.submitFormWrapper}>
                        {formInputs.map((input, key)=>(
                            <View key={(key+1)} style={styles.submitFormRow}>
                                <View style={[styles.submitFormCol, {alignItems: 'flex-start'}]}>
                                    <TextInput
                                        value={input.userId}
                                        ...
                                        textAlign="center"
                                        returnKeyType="next"
                                        style={[styles.formNumberInputStyle, {fontSize: 16, borderColor: (input.userIdError!="")?'#e74c3c':'#dfdfdf'}]} />
                                ...
                                <View style={styles.submitFormCol}>
                                    <TextInput
                                        ...
                                        onEndEditing={(e) => handleValidStartAmount(e.nativeEvent.text, key)}
                                        keyboardType="name-phone-pad"
                                        textAlign="center"
                                        returnKeyType="next"
                                        style={[styles.formNumberInputStyle, {color: 'black', borderColor: (input.startAmountError!="")?'#e74c3c':'#dfdfdf'}]} />

                                    {/* {(input != "") ? (
                                        <View style={styles.errorTextViewStyle}>
                                            <Text style={styles.errorTextStyle}>{input}</Text>
                                        </View>
                                    ) : null } */}
                                </View>
                                <View style={[styles.submitFormCol, {alignItems: 'flex-end'}]}>
                                    <TextInput
                                        ...
                                        onEndEditing={(e) => handleValidEndAmount(e.nativeEvent.text, key)}
                                        keyboardType="numeric"
                                        textAlign="center"
                                        returnKeyType="go"
                                        style={[styles.formNumberInputStyle, {color: '#098e09', borderColor: (input.endAmountError!="")?'#e74c3c':'#dfdfdf'}]} />

                                    {/* {(input.endAmountError != "") ? (
                                        <View style={styles.errorTextViewStyle}>
                                            <Text style={styles.errorTextStyle}>{input.endAmountError}</Text>
                                        </View>
                                    ) : null } */}
                                </View>
                            </View>
                    ...
                    <View style={styles.submitFormButtons}>
                        <View style={{ width: '50%', paddingLeft: 0, paddingRight: 8 }}>
                            <Button title={"Add More Line"} type="clear"
                        ...
                        <View style={{ width: '50%', paddingLeft: 8, paddingRight: 0 }}>
                            <Button title={"Submit"} type="clear" raised={true}
